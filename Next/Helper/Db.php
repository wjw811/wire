...
/*{{{ __construct */
    public function __construct() {
        ...
        try {
            ...
            $msg = $e->getMessage();
            // Unknown database: try to create it automatically, then retry
            if (strpos($msg, 'Unknown database') !== false || strpos($msg, '1049') !== false) {
                ...
                try {
                    ...
                    $pdo->exec("CREATE DATABASE IF NOT EXISTS `" . str_replace('`','',$dbname) . "` DEFAULT CHARACTER SET " . $charset);
                    // retry connect
                    $this->medoo = new Medoo($options);
        ...
        $this->connect["default"] = $this->medoo;
    }
/*}}}*/
/*{{{ __call */
...
/*{{{ init */
    public function init($name) {
        if (isset($this->connect[$name])) {
            $this->medoo = $this->connect[$name];
            return $this;
        }

        $config = $this->app->config('mysql');
        if (!isset($config[$name])) {
            throw new \RuntimeException('Connect is not found.');
        }
        $config = $config[$name];
        $options = [
            'database_type' => 'mysql',
