...
func newConn(id int64, conn *net.TCPConn, srv *Server) *Conn {
	return &Conn{
		ID:                id,
		srv:               srv,
		conn:              conn,
		closeChan:         make(chan struct{}),
		packetSendChan:    make(chan Packet, srv.config.PacketChanLimit),
		packetReceiveChan: make(chan Packet, srv.config.PacketChanLimit),
	}
}

// GetServer gets the server from the Conn
...
// ticker
func (c *Conn) Ticker(fn func(int)) {
	asyncDo(func() {
		t := 100
		if c.srv.config.PacketTimer > 0 {
			t = c.srv.config.PacketTimer
		}
...
func (c *Conn) readLoop() {
	...
	for {
		...
		// set timeout
		if c.srv.config.PacketTimeout != 0 {
			c.conn.SetDeadline(time.Now().Add(time.Duration(c.srv.config.PacketTimeout) * time.Second))
		}
