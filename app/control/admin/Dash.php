        ...
        foreach ($dev as $row) {
            ...
            foreach ($row['feature'] as $k) {
                ...
                if (in_array($k, ['f48', 'f49', 'f50', 'f51'])) {
                    ...
                    // ✨ 修复：根据协议，位为0表示有故障。
                    // 只要不是 255 (0xFF)，就认为可能存在故障位，需要前端解析。
                    // 这里设置为 true 确保前端执行 updateFaultBanner 逻辑。
                    if ($v !== 255) {
                        $warn = true;
                    }
                }
                // nil value
                // ✨ 修复：对于故障寄存器 f48-f51，保留原始数值，不替换为 "无"
                // 因为这些数值需要前端进行位解析，且 0 在这里是有效数值（代表位全为0）
                if (in_array($k, ['f52', 'f53'])) {
                    if (!$v) {
                        $v = @$this->app->i18n['nil'];
                    }
                }
